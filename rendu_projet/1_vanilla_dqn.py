"""
VANILLA DQN (Deep Q-Learning)
==============================
Classic DQN from the 2015 Nature paper.
Uses a single target network, but action selection and evaluation
both use the target network (can lead to overestimation).
"""
...
MODEL_NAME = "vanilla_dqn_model.pt"
...
print(f"[VANILLA DQN] Using device: {device}")
...
def main():
    ...
    print(f"[VANILLA DQN] Starting training for {MAX_EPISODES} episodes")
    print("="*60)
    ...
    while episode < MAX_EPISODES:
        ...
        # Training step (VANILLA DQN - uses target_net for both action selection and evaluation)
        ...
        # VANILLA DQN: Use target_net for next Q-values (max over actions)
        with torch.no_grad():
    ...
    env.close()
    print("="*60)
    print(f"[VANILLA DQN] Training complete! Best eval reward: {best_eval_reward:.1f}")
